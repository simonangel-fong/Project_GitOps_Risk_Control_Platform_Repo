# Debug: ExternalDNS creates no records in Cloudflare
...
A running pod with no records means either (a) it can't talk to Cloudflare, (b) the API token lacks permissions, or (c) it isn't discovering any desired hostnames. Check the logs first — they distinguish all three.
...
Two compounding issues in [bootstrap/40_external-dns.yaml](../bootstrap/40_external-dns.yaml):

1. **Sources only included `service`.** The hostname `gitops-dev.arguswatcher.net` is declared on the HTTPRoute ([apps/frontend/base/40_httproute.yaml](../apps/frontend/base/40_httproute.yaml)), not on any Service. ExternalDNS won't see HTTPRoute hostnames unless `gateway-httproute` is in `sources`.
...
Add `gateway-httproute` to the source list. ExternalDNS will then walk every HTTPRoute, follow `parentRefs` to the Gateway, read the Gateway's `status.addresses` (the NLB hostname), and create a CNAME per HTTPRoute hostname.

```yaml
# bootstrap/40_external-dns.yaml
sources:
  - service
  - gateway-httproute
```

Confirm after Argo sync (pod restarts with new sources):
...
# Debug: HTTPS returns "unable to get local issuer certificate"
...
The ACM cert wired into the NLB ([platform/envoy/gateway.yaml](../platform/envoy/gateway.yaml)) is a **Cloudflare Origin CA cert**, only trusted by Cloudflare's edge proxy — not by browsers or `curl`. It was designed for the CF-edge → origin leg, not for direct client → origin traffic. Because the Cloudflare DNS record was unproxied (grey cloud), clients hit the NLB directly and saw the untrusted Origin cert.
...
1. **ExternalDNS default**: add `--cloudflare-proxied` in [bootstrap/40_external-dns.yaml](../bootstrap/40_external-dns.yaml).
...
2. **Per-route override**: annotate the HTTPRoute in [apps/frontend/base/40_httproute.yaml](../apps/frontend/base/40_httproute.yaml).
...
3. **Cloudflare zone setting**: SSL/TLS → Overview → **Full (strict)**. This makes CF validate the origin cert against the Origin CA — the Origin cert is only honored in this mode.
