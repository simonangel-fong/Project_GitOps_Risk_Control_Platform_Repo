# Project: GitOps Canary Promotion

**End to End. Auto-Promoted. Canary-Released**

> A production-style GitOps project that separates application, infrastructure, and platform delivery across 3 repositories. 
> It uses EKS, ArgoCD, Argo Rollouts, Terraform, and GitHub Actions to automate environment-based deployments, canary promotion, rollback, and post-deployment monitoring.
...
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=for-the-badge&logo=prometheus&logoColor=white&style=plastic) ![Grafana](https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white&style=plastic) ![Alertmanager](https://img.shields.io/badge/Alertmanager-E6522C?style=for-the-badge&logo=prometheus&logoColor=white&style=plastic) ![Slack](https://img.shields.io/badge/Slack-4A154B?style=for-the-badge&logo=slack&logoColor=white&style=plastic)

<!-- ![Checkov](https://img.shields.io/badge/Checkov-5C4EE5?style=for-the-badge&logo=checkmarx&logoColor=white&style=plastic) -->

- [Project: GitOps Canary Promotion](#project-gitops-canary-promotion)
  - [1. Why This Project Exists](#1-why-this-project-exists)
    - [Challenge 1: Managing App, Infrastructure, and Platform Changes](#challenge-1-managing-app-infrastructure-and-platform-changes)
    - [Challenge 2: Releasing Safely Without Business Interruption](#challenge-2-releasing-safely-without-business-interruption)
...
Modern software delivery requires more than deploying application code. Teams also need to manage infrastructure, platform services, release control, monitoring, and rollback in a consistent way.

### Challenge 1: Managing App, Infrastructure, and Platform Changes

In enterprise environments, application code, cloud infrastructure, and Kubernetes platform configuration are often owned by different roles. Without clear separation and automated GitOps workflows, delivery can become slow, inconsistent, and difficult to audit.

**Response:**  
This project uses a 3-repo GitOps strategy to separate application, infrastructure, and platform responsibilities. CI/CD pipelines automate validation, image delivery, infrastructure provisioning, and manifest updates, making the delivery process more traceable and repeatable.

---

### Challenge 2: Releasing Safely Without Business Interruption

Direct production releases increase the risk of downtime, failed deployments, and slow recovery. Without a controlled deployment strategy, issues may only be detected after they affect users.

**Response:**  
This project implements canary deployment across isolated `dev`, `stage`, and `prod` environments. Argo Rollouts, automated analysis, monitoring, and rollback logic help detect issues early, reduce release risk, and protect business continuity.

---

## 2. Project Architecture

This project uses a 3-repo GitOps model to separate application delivery, infrastructure provisioning, and platform configuration.
...
Argo Rollouts Starts Canary Release
        |
        v
Shift Small Portion (25%) of Traffic to New Version
        |
        v
...
- Scenario: Failure rollback on DB connection error
...
This repo uses **ArgoCD Sync Waves** to deploy resources layer by layer: lower wave numbers run first, higher wave numbers run later.
...
This project uses automated pipelines to validate platform changes, promote manifests across environments, and keep production releases approval-based.

| Env     | Owner / Trigger                                         | Jobs Automated by Pipeline                                                                                          |
| ------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `dev`   | Platform Engineer commits manifest changes              | Manifest linting, security scan, GitOps sync, smoke test, failure notification                                      |
...
- The pipeline keeps `dev` and `stage` highly automated for fast validation, while `prod` requires release approval to protect production stability.

---

### 3.4 Environment Strategy

This project separates `dev`, `stage`, and `prod` to support fast validation, production-like testing, and controlled production release. Each environment has a clear purpose, so changes can move through the delivery flow with lower risk.

- The infrastructure repo provisions a separate EKS cluster for each environment
- This platform repo manages the GitOps branch, manifests, and promotion flow for each cluster.

| Env     | Branch  | Cluster Name   | Purpose                                            | Feature                            |
| ------- | ------- | -------------- | -------------------------------------------------- | ---------------------------------- |
...
| -------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Debugging ExternalDNS                  | [`docs/debug_edns.md`](docs/debug_edns.md)                           | Common ExternalDNS sync issues and troubleshooting steps                |
| Debugging AWS Load Balancer Controller | [`docs/debug_lb.md`](docs/debug_lb.md)                               | Common ALB / load balancer misconfigurations and fixes                  |
| Debugging ArgoCD Sync Waves            | [`docs/argocd_wave.md`](docs/argocd_wave.md)                         | Sync wave ordering and dependency control                               |
| Debugging Slack Notifications          | [`docs/debug_argo_notification.md`](docs/debug_argo_notification.md) | Slack notification setup and common issues for ArgoCD and Argo Rollouts |
...
| ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Separate clusters improve isolation but increase operational cost and management overhead. | Add cost controls, shared platform modules, and automated cleanup for non-prod environments.                         |
| Canary rollback depends on selected health checks and metrics.                             | Expand analysis with stronger SLO-based metrics such as error rate, latency, saturation, and business-level signals. |
| Production promotion still requires manual approval.                                       | Integrate richer release evidence, approval history, and change-management records before production deployment.     |
| The project demonstrates platform automation but does not cover full disaster recovery.    | Add backup, restore, multi-AZ / multi-region failover testing, and recovery runbooks.                                |
